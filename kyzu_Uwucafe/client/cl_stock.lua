...
-- Menu --
local open = false
local MenuStockUwucafe = RageUI.CreateMenu("Stock", "INTERACTION")
MenuStockUwucafe.Display.Header = true
MenuStockUwucafe.Closed = function()
    open = false
end

function OpenMenuStockUwucafe()
    if open then
        open = false
        RageUI.Visible(MenuStockUwucafe, false)
        return
    else
        open = true
        RageUI.Visible(MenuStockUwucafe, true)
        CreateThread(function()
            while open do 
                RageUI.IsVisible(MenuStockUwucafe, function()
