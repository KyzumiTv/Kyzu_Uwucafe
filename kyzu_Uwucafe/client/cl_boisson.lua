...
-- Menu --
local open = false
local MenuBoissonUwucafe = RageUI.CreateMenu("Boisson", "INTERACTION")
MenuBoissonUwucafe.Display.Header = true
MenuBoissonUwucafe.Closed = function()
    open = false
end

function OpenMenuBoissonUwucafe()
    if open then
        open = false
        RageUI.Visible(MenuBoissonUwucafe, false)
        return
    else
        open = true
        RageUI.Visible(MenuBoissonUwucafe, true)
        CreateThread(function()
            while open do 
                RageUI.IsVisible(MenuBoissonUwucafe, function()
                    RageUI.Separator("↓    ~o~Boisson    ~s~↓")
                    for k, v in pairs(Config.Boisson) do
                        RageUI.Button(v.Nom, nil, {RightLabel = "~y~→"}, true, {
                            onSelected = function()
                                TriggerServerEvent('Uwucafe:GiveItem', v.Nom, v.Item)
                            end
                        })
                    end
