...
-- Menu --
local open = false
local MenuCuisineUwucafe = RageUI.CreateMenu("Cuisine", "INTERACTION")
MenuCuisineUwucafe.Display.Header = true
MenuCuisineUwucafe.Closed = function()
    open = false
end

function OpenMenuCuisineUwucafe()
    if open then
        open = false
        RageUI.Visible(MenuCuisineUwucafe, false)
        return
    else
        open = true
        RageUI.Visible(MenuCuisineUwucafe, true)
        CreateThread(function()
            while open do 
                RageUI.IsVisible(MenuCuisineUwucafe, function()
                    ...
                    for k, v in pairs(Config.Preparation) do
                        RageUI.Button(v.Nom, nil, {RightLabel = "~y~→"}, true, {
                            onSelected = function()
                                ...
                                TriggerServerEvent('Uwucafe:Preparation', v.Nom, v.ItemRequis, v.ItemCuisiner)
                            end
                        })
                    end
